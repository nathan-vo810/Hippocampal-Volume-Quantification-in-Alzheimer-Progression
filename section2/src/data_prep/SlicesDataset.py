...
class SlicesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image_no, slice_no = slc                                # Load infomation of that slice

        slice_image  = self.data[image_no]['image'][slice_no]   # Load slice image
        slice_image = slice_image[None, :]                      # Reshape image

        slice_seg = self.data[image_no]['seg'][slice_no]        # Load slice seg
        slice_seg = slice_seg[None, :]                          # Reshape seg

        sample['image'] = torch.from_numpy(slice_image)         # Convert image to torch tensor
        sample['seg'] = torch.from_numpy(slice_seg)             # Convert seg to torch tensor
